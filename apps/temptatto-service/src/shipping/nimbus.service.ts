...
// Default box dimensions for a tattoo sheet envelope (cm / grams)
const BOX_L = 20;
const BOX_B = 15;
const BOX_H = 2;
const BOX_WEIGHT_G = 100;
const PRODUCT_HSN = '4911'; // Other printed matter
...
export class NimbusService {
  ...
  async createShipment(params: {
    ...
    const wName = this.config.get<string>('NIMBUSPOST_WAREHOUSE_NAME', 'Temptatto');
    const wPerson = this.config.get<string>('NIMBUSPOST_WAREHOUSE_PERSON', 'Temptatto');
    ...
    const body = {
      ...
      invoice: [
        {
          invoice_number: `TMPT-${params.orderId.slice(-8).toUpperCase()}`,
